...
export default {
  watch: {
    data(val) {
      ...
      this.searchright = "";
      this.inputRightData = this.inputRighCloneData;
    },
    ...
    "realtime.initOutInputData"(val) {
      this.inputLeftData = this.getMapCheckedDataLeft(
        _.map(val, item => {
          return {
            ...
            type: item.type,
            nativeColumn: item.nativeColumn ? item.nativeColumn : false
          };
        })
      );
      ...
      if (this.data.step.type != "source") {
        this.modifyRightTableData(this.inputLeftData);
      }
    },
    ...
    inputRightData(val, oldV) {
      let checkedOutData = _.map(val, item => {
        return {
          ...
          name: item.name,
          type: item.type,
          nativeColumn: item.nativeColumn
        };
      });
  ...
  methods: {
    ...
    modifyRightTableData(inputLeftData) {
      this.inputRightData = _.map(this.inputRightData, item => {
        if (this.findItem(inputLeftData, item.name)) {
          return {
            ...this.findItem(inputLeftData, item.name),
            alias: item.alias
          };
        } else {
          return item;
    ...
    findItem(inputLeftData, name) {
      return _.find(inputLeftData, item => {
    ...
    addInputLeft() {
      ...
      //导入已选字段
      // this.inputRightData = _.cloneDeep(allMultipleSelection);
      this.inputRightData = [
        ...this.inputRightData,
        ..._.cloneDeep(allMultipleSelection)
      ];
      this.setInputRighCloneData(allMultipleSelection);
      this.multipleSelection = [];
      this.allMultipleSelection = [];
    },
