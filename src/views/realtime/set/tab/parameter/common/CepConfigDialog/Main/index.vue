...
export default {
  watch: {
    ...
    "realtime.FLOW_CEP_DATA_STATA"(val){
        ...
        if(val.cepGroups){
          this.SET_FLOW_DATA_GROUP_ACTION(val.cepGroups);
        }
    }
  },
  props: {
  ...
  destroyed: function() {},
  methods: {
    ...mapActions(["FLOW_CEP_DATA_ACTION","SET_FLOW_DATA_GROUP_ACTION"]),
    getCurrentNode(data) {
      return {
    ...
    handleDrop(val) {
      this.steps.push({
        ...
        id: val.drawIcon.eventType=="GroupPatternEvent"?val.drawIcon.flowid:val.drawIcon.eventType + "_cep_" + (this.steps.length + +1),
        x: event.offsetX,
        y: event.offsetY
      });
    }
