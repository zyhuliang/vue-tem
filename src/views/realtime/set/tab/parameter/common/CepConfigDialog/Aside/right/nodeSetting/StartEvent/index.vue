  ...
  <div class="cep-start">
    <el-form ref="form" :model="form" :label-position="'left'">
      ...
      >
        ...
        <el-input
          v-if="(item.type=='String'||item.type=='long'||item.type=='char'||item.type=='Long')&&!item.values"
        ...
        <el-select
          ...
          placeholder="请选择"
          style="width:100%"
          @change="modifyselectdata(item.name,form[item.name])"
        >
          <el-option
...
export default {
  ...
  methods: {
    ...
    modifyselectdata(key, value) {
      console.log("modifyselectdata(key, value) {", key, value);
      if (key != "skipStrategy") {
        return;
      }

      console.log("this.cloneformItemList", this.cloneformItemList);

      this.formItemList = remove(cloneDeep(this.cloneformItemList), item => {
        if (value != "SKIP_TO_FIRST" && value != "SKIP_TO_LAST") {
          return item.name != "skipToPatternName";
        } else {
          return item;
        }
      });
    ...
    modifyformItemList(strategy) {
      ...
      this.formItemList = remove(this.formItemList, item => {
    ...
    initForm(val) {
      ...
      //this.modifyselectdata("skipStrategy", this.form["skipStrategy"]);
    },
