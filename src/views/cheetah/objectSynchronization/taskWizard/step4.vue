<template>
    <div class="step4"  id='bm-ch-col-step4-tmp'>
        <el-form id="bm-sh-step4-form" label-position="top" :model="step4Form" ref="step4Form" :rules="rules" class="demo-step4">
            <el-form-item label="触发器" :label-width="formLabelWidth" prop="sliceType">
                <el-select name="bm-ch-step4Form-trigger-select"  class='select-form-width' v-model="trigger"  @change="triggerChange(trigger)">
                    <el-option
            ...
            <!-- &lt;!&ndash; 表单项，prop属性表示要进行表单验证，验证规则对应为rules的属性name &ndash;&gt;
             <el-form-item type="number" label="并行度" :label-width="formLabelWidth"  prop="parallelism" >
                 <el-input name="bm-ch-step4Form-parallelism-input" type="text" v-model="step4Form.parallelism" auto-complete="off"></el-input>
             </el-form-item>

             <el-form-item label="错误记录上限数" :label-width="formLabelWidth"  prop="errorNumber" >
                 <el-input name="bm-ch-step4Form-errorNumber-input" type="text" v-model="step4Form.errorNumber" auto-complete="off" placeholder="错误记录数超过此值任务自动结束"></el-input>
             </el-form-item>
 ...
 -->
            <el-form-item v-if="trigger == '周期执行' ">
                <span>执行周期：   {{setCron}}</span>
                <el-button name="bm-ch-step4Form-showCron-button" v-show="isModify" type="text" @click="showCron">修改</el-button>
            </el-form-item>
            ...
            <el-form-item label="增量字段" :label-width="formLabelWidth"  prop="cursorCol" v-if="trigger == '周期执行'">
                <el-select name="bm-ch-step4Form-cursorCol-select" type="text" v-model="step4Form.cursorCol" auto-complete="off" placeholder="增量字段">
                    <el-option key=""
                               label=""
                               value="">
                    </el-option>
            ...
            <!-- <el-form-item label="执行参数" :label-width="formLabelWidth"  prop="opts">
                 <el-input name="bm-ch-step4Form-opts-input" type="textarea" v-model="step4Form.opts" auto-complete="off" placeholder="执行参数"></el-input>
             </el-form-item>

             <el-form-item label="缓冲区大小" :label-width="formLabelWidth"  prop="bufferSize">
                 <el-input name="bm-ch-step4Form-bufferSize-input" type="text" v-model="step4Form.bufferSize" auto-complete="off" placeholder="缓冲区大小"></el-input>
             </el-form-item>

             <el-form-item label="延时刷出时间(毫秒)" :label-width="formLabelWidth"  prop="flushPaddingTime">
                 <el-input name="bm-ch-step4Form-flushPaddingTime-input" type="text" v-model="step4Form.flushPaddingTime" auto-complete="off" placeholder="延时刷出时间"></el-input>
             </el-form-item>-->
        </el-form>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        data: function () {
            return {
                dateFields:[],//增量字段只显示data和timepstamp
                //设置form元素左侧距离的变量
        ...
        methods: {
            ...
            init: function () {
                //如果是周期执行就显示周期
                if(this.step4Form.trigger != ''){
                    this.setCron = cronstrue.toString(this.step4Form.trigger, { locale: "fr"})
                    this.isModify = true
                    this.trigger = "周期执行"
                }else {
                    this.trigger = "单次执行";
        ...
        created(){
            if(localStorage.taskEntity){
                //获取全局的entity
                this.step4Form = JSON.parse(localStorage.taskEntity);
                // 根据sft转换出dateFields
                this.dateFields = []
                if(this.step4Form.fields && this.step4Form.fields.sft){
                    var that = this;
                    that.step4Form.fields.sft.forEach(function (item){
        ...
        beforeRouteLeave: function (to, from, next) {
            if(localStorage.taskEntity){
                ...
                this.step4Form = this.copyObject(entity, this.step4Form)
                //保存全局的entity到storage
                localStorage.taskEntity = JSON.stringify(this.step4Form);
            }
            //如果在向导中跳转
