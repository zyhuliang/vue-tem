<template>
    <div class="step4" id='bm-ch-filesys-step4-tmp'>
        <el-form id="bm-ch-file-step4-form" label-position="top" :model="step4Form" ref="step4Form" :rules="rules"
                 class="demo-step4 demo-form-stacked">
            <!-- 表单项，prop属性表示要进行表单验证，验证规则对应为rules的属性name -->
            <el-form-item type="number" label="并行度" :label-width="formLabelWidth" prop="parallelism" required>
                <el-input name="bm-ch-file-step4Form-parallelism-input" type="text" v-model="step4Form.parallelism"
                          auto-complete="off"></el-input>
            </el-form-item>

            <el-form-item label="错误记录上限数" :label-width="formLabelWidth" prop="errorNumber" required>
                <el-input name="bm-ch-file-step4Form-errorNumber-input" type="text" v-model="step4Form.errorNumber"
            ...
            <el-form-item label="Schema发生改变后，此任务应该" :label-width="formLabelWidth"
                ...
                <el-radio-group name="bm-ch-file-step4Form-stopOnSchemaChanged-input" prop="stopOnSchemaChanged"
            ...
            <el-form-item label="触发器" :label-width="formLabelWidth" prop="sliceType">
                <el-select name="bm-ch-file-step4Form-trigger-select" class='select-form-width' v-model="trigger"
            ...
            <el-form-item v-if="trigger == '周期执行' && dsType != 'SOCKET' ">
                ...
                <el-button name="bm-ch-file-step4Form-showCron-button" v-show="isModify" type="text" @click="showCron">
                    修改
                </el-button>
            </el-form-item>
            ...
            <el-form-item label="结束时间" v-if="trigger == '周期执行' && dsType != 'SOCKET' ">
                <div class="block">
                    <el-date-picker
                            v-model="endTime"
                            type="datetime"
            ...
            <!-- <el-form-item label="增量字段" :label-width="formLabelWidth"  prop="cursorCol" v-if="trigger == '周期执行' && dsType != 'SOCKET'">
                 <el-select type="text" v-model="step4Form.cursorCol" auto-complete="off" placeholder="增量字段">
                     <el-option
                             v-for="item in sf"
                             :key="item"
                             :label="item"
             ...
             <el-form-item label="分区键" :label-width="formLabelWidth"  prop="partitionKey" >
                 <el-select type="text" v-model="step4Form.partitionKey" auto-complete="off" placeholder="分区键"  @change = "partitionKeyType(step4Form.partitionKey)">
                     <el-option
                             v-for="item in st"
                             :key="item"
                             :label="item"
            ...
            <!--<el-form-item label="分区键格式" :label-width="formLabelWidth"  prop="partitionPattern" v-if="trigger == '周期执行' && dsType != 'SOCKET' && isDate">
                样例: {{partitionKeyText}}
                <el-input name="bm-ch-file-step4Form-parallelism-input"  type="textarea" v-model="step4Form.partitionPattern" auto-complete="off" placeholder="yyyy-MM-dd" @blur="changePartitionKeyFormat(step4Form.partitionPattern)"></el-input>
            </el-form-item>-->
        </el-form>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            partitionKeyType: function (partitionKey) {
                var that = this;
                that.isDate = false;
                that.step4Form.mappingSource.forEach(function (e) {
                    if (e.sourceField == partitionKey && (e.sourceType.toLowerCase() == "date" || 'timestamp' == e.sourceType.toLowerCase().substr(0, 9))) {
                        that.step4Form.partitionPattern = "yyyyMMdd";
