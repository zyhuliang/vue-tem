<!-- steps参数 dataFlow Datamining naivebayes_train -->
<template>
    <div class="stepTable">
        <el-form  ref="nodeSetData" :model="nodeSetData" label-width="150px" id="fpgrowth">
            <!-- modelType -->
            <el-form-item :label="'modelType'| cn">
                <el-tooltip  class="item" effect="dark" content="modelType">
                    <i class="parIcon el-icon-warning"></i>
                </el-tooltip>
                <el-select class="selectClass" :value="nodeData.modelType " placeholder="modelType（必填，多选）" @change="modelTypeSelect">
                    <el-option value="multinomial">multinomial</el-option>
                    <el-option value="bernoulli">bernoulli</el-option>
                </el-select>
            </el-form-item>
            <i class="sign" v-if="nodeData.modelType ==''">{{'modelType'| cn}}不能为空</i>
            <!-- modelPath -->
...
<script>
    ...
    export default {
        ...
        methods: {
            // modelType 下拉框获取值
            modelTypeSelect:function (val) {
                this.nodeData.modelType = val
            },
            // modelPath 绑定获取值
