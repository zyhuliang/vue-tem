<!-- steps参数 dataFlow Datamining linearleastsquares_predict -->
<template>
    <div class="stepTable">
        <el-form  ref="nodeSetData" :model="nodeSetData" label-width="150px" id="bm-Datamining-predict">
            ...
            <el-form-item :label="'featureColumns '| cn">
                ...
                <el-select class="selectClass" multiple filterable allow-create  default-first-option :value="nodeData.featureColumns " placeholder="（必填，多选）" @change="featureColumnsSelect">
                    <i v-if="getSelFields.length!=0">
                        <el-option v-for="(lb,inputLb,index) in getSelFields[0].fields" :key="inputLb" :value="lb.alias" :name="lb.alias"></el-option>
                    </i>
                </el-select>
            </el-form-item>
