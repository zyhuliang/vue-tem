        ...
        <div class="stepTable">
                <el-form id="bm-nodeset-top-form" ref="nodeSetData" :model="nodeSetData" label-width="150px">
                        ...
                        <el-form-item label="groupBy">
                                ...
                                <el-select class="selectClass" multiple filterable allow-create default-first-option :value="nodeData.groupBy" @visible-change="groupByFocus($event)" placeholder="请选择（多选）" @change="groupBySelect">
                                        <div v-if="getSelFieldData !== '' || getSelFieldData.length != 0">
                        ...
                        <el-form-item :label="'orderBy'| cn">
                                ...
                                <el-select class="selectClass" multiple filterable allow-create default-first-option :value="nodeData.orderBy" placeholder="请选择（多选）" @visible-change="orderByFocus($event)" @change="orderBySelect">
                                        <div v-if="getSelFieldDatas !== '' || getSelFieldDatas.length != 0">
                                                <el-option v-for="(lb,inputLb,index) in getSelFieldDatas[0].fields" :key="inputLb" :value="lb.alias+' asc'"></el-option>
                                                <el-option v-for="(lc,inputLc,index) in getSelFieldDatas[0].fields" :key="index" :value="lc.alias+' desc'"></el-option>
                                        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            orderByFocus:function(callback){
                var _this = this;
                if(callback == true){
                    _this.getSelFieldDatas = JSON.parse(JSON.stringify(_this.getSelFields));
                }else{
                    _this.getSelFieldDatas = [{id: "input",fields:[]}]
                }
            },
