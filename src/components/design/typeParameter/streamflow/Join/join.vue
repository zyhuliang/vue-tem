    ...
    <div class="stepTable">
        <el-form id="bm-nodeset-join-form" ref="nodeSetData" :model="nodeSetData" label-width="150px">
            <el-form-item label="joinType">
                ...
                <el-select class="selectClass" :value="nodeData.joinType" placeholder="请选择" @change="joinTypeSelect">
            ...
            <el-form-item label="joinConditions">
                ...
                <el-table :data="joinConditionsArr.slice((currentPage-1)*pagesize,currentPage*pagesize)" style="width: 100%"
                    ...
                    <el-table-column prop="leftColumn" label="左(字段)">
                        <template slot-scope="scope">
                            <p v-if="getSelFields[0]">
                                 <el-select class="selectClass" :value="scope.row.leftColumn" placeholder="字段（必填）" @change="(value) => leftColumn(value,scope.$index,joinConditionsArr)">
                                    <el-option v-for="(lf,inputLf,index) in getSelFields[0].fields" :key="inputLf" :value="lf.column"></el-option>
                                </el-select>
                            </p>
                    ...
                    <el-table-column prop="op" label="运算符">
                        <template slot-scope="scope">
                            <el-select class="selectClass" :value="scope.row.op" placeholder="函数（必填）" @change="(value) => op(value,scope.$index,joinConditionsArr)">
                    ...
                    <el-table-column prop="rightColumn" label="右(字段)">
                        <template slot-scope="scope">
                            <p v-if="getSelFields[0]">
                                <el-select class="selectClass" :value="scope.row.rightColumn" placeholder="别名（必填）" @change="(value) => rightColumn(value,scope.$index,joinConditionsArr)">
                                    <el-option v-for="(lr,inputLr,index) in getSelFields[1].fields" :key="inputLr" :value="lr.alias"></el-option>
                                </el-select>
                            </p>
                        </template>
                    </el-table-column>
...
<script>
    export default {
        ...
        props: ["nodeSetData", "getSelFields","selectOptions"],
        watch: {
