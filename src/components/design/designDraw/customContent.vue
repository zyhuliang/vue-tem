...
<script>
    ...
    import {queryDefinStep, deleteDefinStep} from "@api"
    ...
    export default {
        ...
        methods: {
            ...
            desSetSubmit() {                //确认
                this.customDataSet.flowSetVisible = false;
                if(typeof this.customContentData !== 'undefined'){
                    this.$emit('customDataContentSet', this.customContentData)
                } 
            },
            deleteStep(val){
                var IdArr = []
                ...
                deleteDefinStep(IdArr).then( (data)=>{
                    var parameter=queryObject()
                      queryDefinStep(parameter, 'dataflow').then( (data) => {
                          _this.json1 = data
                      })
                })
            },
           ...
           searchTable(){
            ...
            listTable(parameter){
                var _this = this
                queryDefinStep(parameter, this.type).then( (data) => {
                    _this.json1 = data
                })
            }
        },
