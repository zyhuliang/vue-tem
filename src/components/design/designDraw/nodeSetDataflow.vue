                ...
                <el-form label-width="170px" v-if="nodeSetFlowType == 'sink'">
                  <el-collapse id="advancedOptions" accordion>
                    <el-collapse-item>
                      ...
                      <el-form-item
                        v-if="nodeSetFlowType == 'sink' && (nodeSetData.otherConfigurations.type=='HDFS'||nodeSetData.otherConfigurations.type=='S3A')"
                        :label="'nullValue'| cn"
                      >
                        <el-tooltip
                          class="item"
...
import { nodeComp, nodeClass, customizeSteps } from "@function/nodeSet";
    ...
    nodeSet: {
      handler(newValue) {
        ...
        if (val.dbStatus == "1") {
          ...
          this.nodes = comp[customizeSteps(this.nodeSetFlowType)];
...
//                   }
//                 }
// >>>>>>> 6653f596fff723129af63e961f76019620252a15
                _this.getDataFlow(_this.nodeSet.flowData);
              }
            }
          },
          ...
          data => {
            if (
              ...
              if (
                ...
                customizeSteps(this.dataflow.dblSteps.designType) == "customize"
              ) {
    ...
    handleClick(tab, event) {
      ...
      if (ouputClassName === "参数") {
        if (customizeSteps(this.dataflow.dblSteps.designType) == "customize") {
          defineConfDetail(this.dataflow.dblSteps.designType).then(data => {
            _this.setSetting(data.settings);
            if (JSON.stringify(_this.nodeSetData.otherConfigurations) == "{}") {
              _this.nodeSetData.otherConfigurations = data.settings;
              let val = _this.nodeSetData.otherConfigurations;
              var settingJson = {};
              for (var i = 0; i < val.length; i++) {
                settingJson[val[i].fileName] = val[i].defaultValue
                  ? val[i].defaultValue
                  : "";
              }
              _this.nodeSetData.otherConfigurations = settingJson;
            }
            _this.nodeSetData.otherConfigurations = JSON.parse(
              JSON.stringify(_this.nodeSetData.otherConfigurations)
            );
            _this.nodeSetData.otherConfigurations.customJarFileName =
              data.customJarFileName;
            _this.nodeSetData.otherConfigurations.stepClassName =
              data.stepClassName;
          });
        }
        ...
        if (_this.nodeSetFlowType == "source" && ouputClassName !== "参数") {
          ...
          if (
            (_this.nodeSetFlowType == "minus" ||
              ...
              (customizeSteps(this.dataflow.dblSteps.designType) ==
                "customize" &&
                ouputClassName.indexOf("输出") == "-1")) &&
            ouputClassName != "参数"
          ) {
      ...
      if (
        ...
        var index = ouputClassName.indexOf("输出");
        var start = ouputClassName.indexOf("(");
        var end = ouputClassName.indexOf(")");
        var outputid = ouputClassName.slice(start + 1, end);
        // 输出是一个数组
        this.loading = true;
        errorOutputDataFlow(outputid, this.nodeSetData).then(
          data => {
            this.statementId = data.statementId;
            this.clusterId = data.clusterId;
            this.getDefaultStepsOut();
