            ...
            <ul class="tree-parent">
                <li>
                    <ul class="ulP" v-for="(tree,key,index) in theModel">
                        <dataset-menu  ref="datasetMenu" :model="tree"></dataset-menu>
                    </ul>
                </li>
            </ul>
        </div>
...
<script>
    import datasetMenu from './datasetTreeMenu.vue'
    ...
    export default {
        ...
        mounted() {
            //初次获取资源目录schema树的参数
            this.parameter = { excludes: "", includes: 'dataset_dir', strict:true};
            this.getTree(this.parameter)
            datasetJs.$on('getNode',data=>{
