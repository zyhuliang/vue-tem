...
<template>
    <li class="treeBox">
        ...
        <em @click="schemaNavStyle(model)" ref="schemaInqDataCss" class="schemaInqActive">{{ model.name|nameFilter }}&nbsp;{{ model.children.length | lengthFilter}}</em>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            schemaNavStyle:function (dataNavId) {
                var arr = document.getElementsByClassName("schemaInqActive");
                for(var i = 0 ; i< arr.length; i++){
                    arr[i].style.color = '';
                }
                this.$refs.schemaInqDataCss.style.color = '#3e98fe';
                daIn.$emit('selectSchemaLabel',dataNavId);
            }
        }
    }
</script>
